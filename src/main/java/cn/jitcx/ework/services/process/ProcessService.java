package cn.jitcx.ework.services.process;
 ...
 /**
   ...
   */
	...
	public Page<AubUser> index(User user,int page,int size,String val,Model model){
		...
		User u=udao.findByRealName(val);//找用户
	...
	public void  index6(Model model,Long id,int page,int size){
		...
		model.addAttribute("poslist", poslist);
		model.addAttribute("url", "names");
		model.addAttribute("realName", lu.getRealName());
	 ...
	 */
	public void index5(ProcessList pro, String val, User lu, MultipartFile filePath, String name, FilePath fPath) throws IllegalStateException, IOException{
		...
		if(!StringUtil.isEmpty(filePath.getOriginalFilename())){
			attaid= (Attachment) fileServices.savefile(filePath, lu, fPath, false);
			AttDao.save(attaid);
	...
	public void save(Long proid,User u,Reviewed reviewed,ProcessList pro,User u2){
		...
		pro.getShenuser();
		pro.setShenuser(pro.getShenuser()+";"+u2.getRealName());
