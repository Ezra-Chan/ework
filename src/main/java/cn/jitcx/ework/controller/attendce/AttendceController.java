package cn.jitcx.ework.controller.attendce;
...
import cn.jitcx.ework.common.StringtoDate;
import cn.jitcx.ework.model.dao.attendcedao.AttendceDao;
import cn.jitcx.ework.model.dao.attendcedao.AttendceService;
...
import cn.jitcx.ework.services.user.UserService;
...
public class AttendceController {
	...
	public String Datag(HttpSession session, Model model, HttpServletRequest request) throws InterruptedException, UnknownHostException {
		...
		if(hourminsec.compareTo("00:00:00") >= 0 && hourminsec.compareTo("05:00:00") <0){
			//在0点到5点之间不能签到
			...
			if (count == 0) {
				...
				if(hourminsec.compareTo(end) >= 0){
					//超过17点，且当天无签到记录，则不能签到
					model.addAttribute("error", "2");
				}
				else{
					// 此次点击是上班 就是用来判断该记录的类型
					// 上班id8
					typeId = 8;
					// 上班就只有迟到和正常
