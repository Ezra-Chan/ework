...
import cn.jitcx.ework.model.dao.daymanagedao.DaymanageDao;
...
import cn.jitcx.ework.model.dao.notedao.DirectorDao;
...
import cn.jitcx.ework.model.dao.taskdao.TaskuserDao;
import cn.jitcx.ework.model.dao.user.UserDao;
import cn.jitcx.ework.model.dao.user.UserLogDao;
...
import cn.jitcx.ework.model.entity.task.Taskuser;
import cn.jitcx.ework.model.entity.user.User;
import cn.jitcx.ework.model.entity.user.UserLog;
import cn.jitcx.ework.services.daymanage.DaymanageServices;
...
public class IndexController {
	...
	public String index(HttpServletRequest req,Model model) {
		...
		for (ScheduleList scheduleList : aboutmenotice) {
			if(scheduleList.getIsRemind()!=null&&!scheduleList.getIsRemind()){
...
//				simpleDateFormat.parse(scheduleList.getStartTime()).getTime();  
	...
	private void showalist(Model model, Long userId) {
		...
		if (aList != null) {
			Long aListTypeId = aList.getTypeId();
			if(aListTypeId == 8){
				aListTypeId = aListTypeId + 1;
			}
			String type = typeDao.findname(aListTypeId);
