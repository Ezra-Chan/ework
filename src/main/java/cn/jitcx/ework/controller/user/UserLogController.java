...
import cn.jitcx.ework.model.dao.scheduledao.ScheduleDao;
import cn.jitcx.ework.model.dao.taskdao.TaskDao;
...
import cn.jitcx.ework.services.user.UserLogRecordService;
import cn.jitcx.ework.services.user.UserLogService;
...
public class UserLogController {
	...
	public String test3df(HttpServletResponse response) throws IOException {
		...
		for (User user : uList) {
			if(taskDao.countfinish(7l, user.getUserId())>0){
				hashMap.put(user.getRealName(), taskDao.countfinish(7l, user.getUserId()));
				i++;
			}
		}
	...
	public String test3(@RequestParam(value="page",defaultValue = "0")int page,
			HttpSession session,Model model,
			@RequestParam(value="baseKey",required=false)String basekey,
			@RequestParam(value="time",required=false)String time,
			@RequestParam(value="icon",required=false)String icon) {
