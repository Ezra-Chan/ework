	...
	<div class="box-body no-padding">
		<div class="table-responsive">
			<table class="table table-hover">
				<tr>
					<th scope="col">
						<span class="paixu thisstatus">状态
							<#if status?? && icon??>
					...
					<th scope="col">标题</th>
					<th scope="col">
						<span class="paixu thistime">发布时间
							<#if time?? && icon??>
								<span class="glyphicon ${icon}"></span>
							</#if>
						</span>
				...
				<tr>
					...
					<td>
						<a href="informshow?id=${this.notice_id}&read=${this.is_read}&relationid=${this.relatin_id}" class="label xiugai chakan">
							<span class="glyphicon glyphicon-search"></span>查看
						</a>
						<#if this.contain!=1>
							<#if this.contain==3>
								<a href="forwardother?noticeId=${this.notice_id}" onclick="{return confirm('确定转发给自己的下属吗？');};" class="label xinzeng chakan forwardthis"><span class="glyphicon glyphicon-log-out"></span>转发</a>
							<#else>
								<a href="javascript:void(0);" class="label sheding chakan"><span class="glyphicon glyphicon-log-out"></span>已转发</a>
							</#if>
						</#if>
						<#if this.is_read==0>
						<#else>
