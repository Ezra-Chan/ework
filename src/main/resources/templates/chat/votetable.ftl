							...
							<span class="glyphicon glyphicon-hand-up" style="color: #fff;border-radius: 50%;padding: 3px;background-color: #347ab7;margin-left: 12px;"></span> 
								...
								<span class="likethisnum">
									<#list voteTitle.users as user>
										<a class="usershow" title="${user.user.realName}"
											thisdept="${user.user.dept.deptName}"
											thisrole="${user.user.role.roleName}"
											thistel="${(user.user.userTel)!'空的'}"
											thisemail="${(user.user.eamil)!'空的'}"
											href="javascript:void(0);" style="color: #888;font-size: 12px;">
											<#if user.user.imgPath?? && user.user.imgPath!=''  >
												<img src="/image/${user.user.imgPath}" title="${user.user.realName}" alt="${user.user.realName}" class="small-img"/>
											<#else>
												<img src="images/timg.jpg" title="${user.user.realName}" alt="${user.user.realName}" class="small-img">
											</#if>
										</a>
									</#list>
