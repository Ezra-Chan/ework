	...
	<td>
		<div class="post">
			<div class="user-block">
				<span>
					<a href="#">
						<#if reply.user.imgPath?? && reply.user.imgPath!=''  >
							<img class="big-img" src="/image/${reply.user.imgPath}"/>
						<#else>
							<img class="big-img" src="images/timg.jpg" alt="images"/>
						</#if>
					</a>
				</span>
				<span class="realname">
					<a href="#" class="raply-name">${(reply.user.realName)!''}</a>
					<#if manage??>
					...
					<table class="table table-hover" style="margin-bottom: 0;">
						<tbody>
							...
							<tr>
								<td class="comment-td">
									<a href="#">
										<#if comment.user.imgPath?? && comment.user.imgPath!=''  >
											<img class="big-img" src="/image/${comment.user.imgPath}"/>
										<#else>
											<img class="big-img" src="images/timg.jpg" alt="images"/>
										</#if>
									</a>
								...
								<td>
									<div class="user-block">
										<a href="#" class="raply-name">${(comment.user.realName)!''}: </a>${comment.comment}
										<ul class="list-inline pull-right"
											style="display: block;">
											<li>${comment.time?string('yyyy-MM-dd HH:mm:ss')}</li>
											<li><a href="#" class="label xinzeng thisreply"
												replyId="${reply.replyId}" replyModule="reply" replyName="${comment.user.realName}"><span
													class="glyphicon glyphicon-share-alt"></span>回复</a></li>
											<#if manage??>
											<li>
												<a href="javascript:void(0);" class="label shanchu deletethis" replyId="${comment.commentId}" replyModule="comment">
