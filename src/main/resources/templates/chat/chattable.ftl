		...
		<div class="table-responsive">
			<table class="table table-hover">
				...
				<tr>
					...
					<td>
						<#if me??>
							<a href="/writechat?id=${this.id}" class="label xiugai">
								<span class="glyphicon glyphicon-edit"></span> 修改
							</a>
						</#if>
						<a href="/seediscuss?id=${this.id}&pageNumber=${page.number}" class="label xiugai">
							<span class="glyphicon glyphicon-search"></span>查看
						</a>
						<#if manage??>
						 <a href="javascript:void(0);" discussId="${this.id}" class="label shanchu deletethis">
