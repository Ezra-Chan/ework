				...
				<td class=" col-xs-1">
					<span class="imgs center-block">
						<#if user.imgPath?? && user.imgPath!=''  >
							<img class="profile-user-img img-responsive img-circle" src="/image/${user.imgPath}" style="width: 24px; height: 24px;"/>
						<#else>
							<img class="profile-user-img img-responsive img-circle" src="images/timg.jpg" style="width: 24px; height: 24px;"/>
						</#if>
					</span>
				</td>
				<td class="col-xs-2">${user.dept.deptName}</td>
				<td class=" col-xs-2">${user.position.name}</td>
				<td class="col-xs-2 na">${user.realName}</td>
