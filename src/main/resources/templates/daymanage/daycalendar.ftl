						...
						<div class="box">
							<div class="box-body">
								<div style="border-bottom: 1px solid #f4f4f4;">
									<span class="rcId" style="display: none;"></span>
									<h3>
										<span class="rctitle" style="font-size:16px;"></span>
									</h3>
									<h5 style="color: #999;font-size: 13px;">
										<span>
											发布：
											<span class="rcusername"></span>
											类型：
											<span class="rctype"></span>
										</span>
										<span style="float:right;">
											<span class="rcstarttime"></span>
											至
											<span class="rcendtime"></span>
										</span>
									</h5>
								</div>
								<div style="padding:10px;">
									<span class="rcdescription"></span>
								</div>
							</div>
							<div class="box-footer">
								<a class="updateSchedule label xiugai">
									<span class="glyphicon glyphicon-edit"></span> 修改
								</a>
								<a onclick="{return confirm('删除该记录将不能恢复，确定删除吗？');};"
								   class="deleteSchedule label shanchu">
	...
	$(document).ready(function() {
		...
		$('#calendar').fullCalendar({
			...
			eventClick: function(event) {
				...
				$('#myModal .updateSchedule').attr("href","dayedit?rcid="+id);
				$('#myModal .deleteSchedule').attr("href","dayremove?rcid="+id);
				$('#myModal .rcId').html(id);
